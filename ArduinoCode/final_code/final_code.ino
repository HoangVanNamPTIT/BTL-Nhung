...
#include <ESP32Servo.h> // Yêu cầu cài đặt thư viện ESP32Servo
...
const int relayPins[ROOM_COUNT]  = {32, 33}; // Relay (Quạt)
...
TaskHandle_t controlTaskHandle = NULL; // Thêm dòng này để lưu ID của task Control
...
void executeFanControl(int i, bool state) {
  if (rooms[i].fan != state) { 
    ...
    rooms[i].fan = state;
    Serial.printf("[ACTUATOR] -> QUAT R%d: %s\n", i + 1, state ? "ON" : "OFF");
    flagResetI2C = true; // Chống nhiễu relay lên đường I2C
  }
}
...
void taskSensor(void *pv) {
  float alpha = 0.2;
  for (;;) {
    bool hasDanger = false; // Cờ theo dõi mức độ nguy hiểm
    ...
    if (xSemaphoreTake(dataMutex, portMAX_DELAY)) {
      for (int i = 0; i < ROOM_COUNT; i++) {
        ...
        // NẾU CÓ DẤU HIỆU DANG, BẬT CỜ NGUY HIỂM LÊN
    ...
    // GỬI LỆNH ĐÁNH THỨC TASK CONTROL NGAY LẬP TỨC
    if (hasDanger && controlTaskHandle != NULL) {
      xTaskNotifyGive(controlTaskHandle); // Gửi tín hiệu (Signal)
    }
  ...
  for (;;) {
    if (xSemaphoreTake(dataMutex, portMAX_DELAY)) {
      for (int i = 0; i < ROOM_COUNT; i++) {
        ...
        // Yêu cầu: Còi kêu, Cửa mở, Quạt bật cho đến khi người dùng xác nhận trên App.
        if (strcmp(rooms[i].level, "DANG") == 0 && rooms[i].mode == AUTO) {
          rooms[i].mode = MANUAL; // Ép sang Manual để người dùng phải tự xử lý tắt
          Serial.printf("[ALARM] R%d dat muc DANG! Kich hoat khan cap va khoa che do MANUAL.\n", i + 1);
          
          // Thứ tự ưu tiên kích hoạt: Còi -> Quạt -> Cửa
        ...
        // 2. LOGIC TỰ ĐỘNG BÌNH THƯỜNG (Khi ở mức an toàn hơn)
          ...
          // Ưu tiên tắt/bật còi & quạt trước
          executeBuzzerControl(i, false); 
          executeFanControl(i, isBad);
          
          // Hàm tuyến tính tính góc cửa sổ tỷ lệ thuận với khí gas
          // Khí gas (400 -> 3000) tương ứng góc mở (0 -> 180)
    ...
    // Ngủ 1500ms nhưng sẽ bật dậy ngay lập tức nếu nhận được Notification (khí gas quá cao)
...
void taskLCD(void *pv) {
  ...
  for (;;) {
    // 1. XỬ LÝ NHIỄU I2C KHI ĐÓNG/CẮT RELAY
    if (flagResetI2C) {
      flagResetI2C = false;
      Wire.end(); 
      delay(20);
    ...
    // 3. HIỂN THỊ TRÁNH CHỚP TẮT (Không dùng lcd.clear)
    // Căn lề và nối chuỗi bằng khoảng trắng để ghi đè dữ liệu cũ
    ...
    if (!error) {
      ...
      if (doc.containsKey("mode") && strcmp(doc["mode"], "UPDATE") == 0) {
        if (doc.containsKey("version")) {
          ...
          flagStartOTA = true; // Bật cờ cho loop() thực thi tải Firmware
          Serial.println("[MQTT] Nhan lenh OTA Update version: " + targetOTAVersion);
        }
        return; // Đã nhận lệnh OTA thì thoát hàm, không xử lý lệnh khác
      }
    ...
    if (millis() - lastPublish > 3000) { 
      if (xSemaphoreTake(dataMutex, portMAX_DELAY)) {
        ...
        for (int i = 0; i < ROOM_COUNT; i++) {
          ...
          // Các thuộc tính gửi thêm lên server
          r["fan"] = rooms[i].fan;
...
void performOTAUpdate(String version) {
  ...
  // TẮT WATCHDOG TRƯỚC KHI UPDATE ĐỂ TRÁNH RESET BẤT NGỜ
  disableCore0WDT();
  disableCore1WDT();
  ...
  // Collect headers TRƯỚC KHI GET để check MD5
...
void setup() {
  ...
  // Cấu hình Timer cho thư viện ESP32Servo (Bắt buộc cho ESP32)
  ESP32PWM::allocateTimer(0);
  ESP32PWM::allocateTimer(1);
  ESP32PWM::allocateTimer(2);
  ESP32PWM::allocateTimer(3);

  // Khởi tạo Thiết bị / Pins
  for (int i = 0; i < ROOM_COUNT; i++) {
    // Relay
    ...
    windowServos[i].write(0); // Đóng cửa mặc định lúc khởi động
  ...
  // Phân quyền ưu tiên Task chuẩn xác như phân tích
  xTaskCreate(taskSensor,  "Sensor",  4096, NULL, 4, NULL); // Ưu tiên cao nhất
  xTaskCreate(taskControl, "Control", 4096, NULL, 3, &controlTaskHandle); // Xử lý thiết bị ngoại vi
  xTaskCreate(taskLCD,     "LCD",     4096, NULL, 2, NULL); // Hiển thị UI Local
  xTaskCreate(taskMQTT,    "MQTT",    8192, NULL, 1, NULL); // Giao tiếp mạng
}
